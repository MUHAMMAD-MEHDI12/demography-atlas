...
/** Colours between two theme tokens. Uses perceptual spacing for better distinction. */
export function ramp(from: string, to: string, n = 6): string[] {
  const a = hex(from), b = hex(to);
  return Array.from({ length: n }, (_, i) => {
    const t = i / (n - 1);
    // Use stronger contrast between steps for easier differentiation
    const eased = Math.pow(t, 0.65);
    const c = a.map((v, k) => Math.round(v + (b[k] - v) * eased));
    return `rgb(${c.join(",")})`;
  });
}
