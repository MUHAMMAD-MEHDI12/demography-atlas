...
const pkgeo = readFileSync(new URL("public/data/pk-geo.json", root), "utf8");
...
for (const [entry, html, out, data] of [
  ...
  page = page.replace("</head>", `<script>${data()}window.__WORLD__=${world};window.__SEARCH__=${search};window.__PK_GEO__=${pkgeo};</script>\n</head>`);
